...
public class Skeleton : Enemy
{
    ...
    {
        if (Time.time >= nextFireTime && skeletonAttack.GetBool("IsAttacking"))
        {
            isShooting = true;
            skeletonAttack.SetBool("IsAttacking", true);
    ...
    {
        while(skeletonAttack.GetCurrentAnimatorStateInfo(0).normalizedTime < 1f)
        {
            yield return null;
        }
        Shoot();
    ...
    private IEnumerator ResetShootingState()
    {
        ...
        isShooting = false;
        skeletonAttack.SetBool("IsAttacking", false);
    }
