...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        ProcessMove();
        ProcessJump();
        SwitchEquippedWeapon(swordIsActive);
    }
    ...
    void SwitchEquippedWeapon(bool swordIsActive)
    {
        if (Input.GetKeyDown(KeyCode.R) && !swordIsActive)
        {
            hammer.SetActive(false);
            sword.SetActive(true);
            swordIsActive = true;
        }
        else if (Input.GetKeyDown(KeyCode.R) && swordIsActive)
        {
